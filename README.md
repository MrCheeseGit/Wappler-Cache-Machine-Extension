...
> 2. **Verify** the package landed: `ls node_modules/wappler-cache-machine/package.json` (if this fails, fix registration before copying anything).
...
1. **Register in Wappler** — Project Settings → Extensions → Add → enter `wappler-cache-machine` or this repository's GitHub URL.
...
3. **Verify before copying** (required):
   ```bash
   ls node_modules/wappler-cache-machine/package.json
   ```
...
4. **Copy files into your project** — open the **[npm install assistant](https://www.mrcheese.co.uk/extensions/install/npm)**, select **Cache Machine**, choose **Server Connect**, copy the generated script, and run it from your project root.
...
After you change extension source, run `npm install wappler-cache-machine` again in the project root, run the npm install assistant copy script and restart Wappler.
---
